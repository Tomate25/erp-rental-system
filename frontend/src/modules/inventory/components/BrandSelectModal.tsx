...
import { getBrands, createBrand } from '../services/inventory.api';
...
export const BrandSelectModal: React.FC<BrandSelectModalProps> = ({ onClose, onSelect }) => {
  ...
  useEffect(() => {
    ...
    if (query === '') {
      ...
      setFilteredBrands(brands.filter((b) => b.nombre.toLowerCase().includes(query)));
    }
  }, [searchQuery, brands]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-2xl max-w-md w-full relative max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 shrink-0">
          <div>
            <h3 className="text-sm font-black text-slate-900">Buscar Fabricante / Marca</h3>
            <p className="text-[10px] text-slate-500 mt-0.5">Selecciona o da de alta una marca en el catálogo.</p>
          </div>
        ...
        <div className="my-4 relative shrink-0">
          ...
          <input
            type="text"
            placeholder="Buscar marca (ej: Caterpillar)..."
            value={searchQuery}
        ...
        {/* Sección de creación rápida si no existe */}
        <form onSubmit={handleCreateBrand} className="border-t border-slate-100 pt-4 mt-4 shrink-0 space-y-2">
          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Crear y seleccionar marca nueva</label>
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Nueva marca (ej: Sany)"
              value={newBrandName}
